...
from src.models.reid_models.layers import *
...
class ResNet2(nn.Module):
    def __init__(self, last_stride=2, block=Bottleneck2,layers=[3, 4, 6, 3]):
        self.inplanes = 64
        super().__init__()
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes,'BN', False, False, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes,'BN', False, False,))
