...
import copy
import math
from src.models.reid_models.gem_pool import GeneralizedMeanPoolingP
from torch.nn import functional as F
from .snn_backbone import SNNBackbone
from .vprtempo_backbone import VPRTempoBackbone
...
def make_model(arg, num_class, camera_num, view_num,pretrain=True):
    if arg.MODEL == 'vprtempo_snn':
        model = VPRTempoBackbone(
            num_classes=num_class,
            model_path=arg.vprtempo_model_path,
            input_hw=(arg.vprtempo_input_h, arg.vprtempo_input_w),
            patches=arg.vprtempo_patches,
            embed_dim=arg.vprtempo_embed_dim,
            dropout=arg.vprtempo_dropout,
            freeze_mode=arg.vprtempo_freeze_mode,
        )
        print('===========building VPRTempo SNN===========')
        return model

    if arg.MODEL == 'snn_tiny':
        model = SNNBackbone(
            num_classes=num_class,
            input_hw=(arg.snn_input_h, arg.snn_input_w),
            hidden_dim=arg.snn_hidden_dim,
            embed_dim=arg.snn_embed_dim,
            timesteps=arg.snn_timesteps,
            dropout=arg.snn_dropout,
        )
        print('===========building SNN===========')
        return model
