...
from src.datasets.lreid_dataset.data.dataset import ImageDataset
import numpy as np
import torch
...
def _processed_split_ready(split_dirs):
    for split_dir in split_dirs:
        if not osp.isdir(split_dir):
            return False
        if len(glob.glob(osp.join(split_dir, '*.jpg'))) == 0:
            return False
    return True
...
class IncrementalSamples4cuhksysu(IncrementalPersonReIDSamples):
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False):
        ...
        if _processed_split_ready(required_files_state):
            ...
            os.makedirs(self.processed_dir_train, exist_ok=True)
            os.makedirs(self.processed_dir_combine, exist_ok=True)
            os.makedirs(self.processed_dir_query, exist_ok=True)
            os.makedirs(self.processed_dir_gallery, exist_ok=True)
        ...
        def _crop_store():
            ...
            for key, person_images in tqdm(data_dict.items()):
                for image_path, box, pid_name, pid, im_name, is_hard in person_images:
                    ...
                    box_tuple = tuple(map(int, box.round()))
